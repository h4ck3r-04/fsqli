...
try:
    ...
    try:
        ...
        sys.exit("[!] wrong installation detected (missing modules). Visit 'https://github.com/fsqliproject/fsqli/#installation' for further details")
...
def checkEnvironment():
    try:
        ...
        errMsg += "Please move the fsqli's directory to the other location"
        logger.critical(errMsg)
        raise SystemExit
    ...
    if LooseVersion(VERSION) < LooseVersion("1.0"):
        ...
        errMsg += "newer versions of fsqli with runtime scripts for older "
        errMsg += "versions"
        logger.critical(errMsg)
        raise SystemExit

    # Patch for pip (import) environment
    if "fsqli.fsqli" in sys.modules:
        for _ in ("cmdLineOptions", "conf", "kb"):
            globals()[_] = getattr(sys.modules["lib.core.data"], _)

        for _ in ("FsqliBaseException", "FsqliShellQuitException", "FsqliSilentQuitException", "FsqliUserQuitException"):
            globals()[_] = getattr(sys.modules["lib.core.exception"], _)

def main():
    """
    Main function of fsqli when running from command line.
    """
    ...
    try:
                        ...
                        if conf.crawlDepth and conf.bulkFile:
                            ...
                            for i in xrange(len(targets)):
                                ...
                                try:
                                    ...
                                    if target and not isinstance(ex, FsqliUserQuitException):
    ...
    except (FsqliSilentQuitException, bdb.BdbQuit):
        pass

    except FsqliShellQuitException:
        cmdLineOptions.fsqliShell = False
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            errMsg = "there is a known issue when fsqli is run with ALPHA versions of Python 3.11. "
            ...
            errMsg += " (Reference: 'https://github.com/fsqliproject/fsqli/issues/4572#issuecomment-775041086')"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "detected usage of files from different versions of fsqli"
            logger.critical(errMsg)
            raise SystemExit
        ...
        elif all(_ in excMsg for _ in ("No such file", "fsqli.conf", "Test")):
        ...
        if (getDaysFromLastUpdate() or 0) > LAST_UPDATE_NAGGING_DAYS:
            warnMsg = "your fsqli version is outdated"
        ...
        if cmdLineOptions.get("fsqliShell"):
